...
try {
  if (config.apiKey && !config.apiKey.includes('your_api_key')) {
    ...
    console.warn(
      'Firebase: API key is missing. Authentication features will be disabled. Check your environment variables.',
    );
  }
} catch (error: any) {
...
export { app, auth, analytics, googleProvider, signInWithPopup, signOut, onAuthStateChanged };
export type { User };

...
export function StageDetail({ stageId, onBack, onOpenResources, project }: StageDetailProps) {
  ...
  const handleDownload = () => {
    if (!data) return;
    const checkedList = data.checklist
      .map((item, i) => `[${checkedItems[i] ? 'x' : ' '}] ${item}`)
      .join('\n');
...
Progress: ${Math.round(progress)}%
  ...
  if (stageId === 'build') {
    ...
    if (project.prizeCategory === 'AI/ML Track') {
      ...
      typeSpecific +=
        ' Use Hardhat/Foundry for smart contracts and Ethers.js for frontend integration.';
    }
      ...
      <nav className="border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-auto sm:h-20 py-4 sm:py-0 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center justify-between w-full sm:w-auto gap-4">
            ...
            <h1 className="text-lg sm:text-xl font-extrabold flex-1 text-center sm:text-left">
              {stageInfo?.title}
            </h1>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none text-gray-500 hover:text-gray-900 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all border border-gray-100 hover:border-gray-200 flex items-center justify-center gap-2"
            >
              <Layout className="w-4 h-4" /> <span className="hidden xs:inline">Resources</span>
              <span className="xs:hidden">Lib</span>
            </button>
            <button
              onClick={handleDownload}
              className="flex-1 sm:flex-none bg-gray-900 hover:bg-black text-white px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all"
            >
              Download
            </button>
          </div>
          ...
          <section className="bg-gray-50 rounded-[2.5rem] p-10 border border-gray-100">
            <div className="flex justify-between items-center mb-8">
              ...
              <a
                href={data.toolUrl}
                target="_blank"
                className="text-black font-bold hover:underline flex items-center gap-2"
              >
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-100 shadow-sm mb-6 text-center lg:text-left overflow-hidden relative">
              ...
              <button
                ...
                className="mx-auto lg:mx-0 text-gray-400 text-sm font-bold flex items-center gap-2 hover:text-gray-600"
              >
                {showAlternatives ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2">
                {data.alternatives.map((alt, i) => (
                  <a
                    ...
                    href={alt.url}
                    target="_blank"
                    className="p-4 bg-white rounded-2xl border border-gray-100 hover:border-gray-900/10 hover:shadow-lg transition-all flex justify-between items-center group"
                  >
                    <div>
            ...
            <div className="relative group">
              ...
              {isEdited && (
                <button
                  ...
                  className="absolute bottom-4 sm:bottom-6 right-4 sm:right-6 text-[10px] sm:text-xs font-bold text-gray-400 hover:text-gray-300 underline"
                >
                  Reset to Default
                </button>
              )}
        ...
        <div className="space-y-12">
          ...
          <section className="bg-white rounded-[2rem] border-2 border-gray-900 p-8 shadow-2xl shadow-gray-200">
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {data.checklist.map((item, i) => (
                ...
                >
                  <div
                    className={`mt-0.5 w-6 h-6 rounded-lg border-2 flex items-center justify-center shrink-0 transition-all ${checkedItems[i] ? 'bg-gray-900 border-gray-900 text-white' : 'border-gray-200'}`}
                  >
                    {checkedItems[i] && <Check className="w-4 h-4" />}
                  </div>
                  <span
                    className={`text-sm font-bold leading-tight ${checkedItems[i] ? 'text-gray-400 line-through' : 'text-gray-900'}`}
                  >
                    {item}
                  </span>
                </div>
          ...
          <section className="bg-gray-50 rounded-[2rem] p-8 border border-gray-100">
            ...
            <div className="space-y-4 mb-6 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {comments.length === 0 ? (
                <p className="text-gray-400 text-sm italic py-4 text-center font-medium">
